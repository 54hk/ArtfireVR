...
import com.artfire.ninedraw.vuforia_lib.VideoViewActivity;
import com.artfire.ninedraw.vuforia_lib.tools.LogUtil;
import com.blankj.utilcode.util.FileUtils;

import java.util.concurrent.locks.ReentrantLock;

import static com.artfire.ninedraw.vuforia_lib.MainActivity.REQUEST_CODE_FULL_SCREEN;
...
//    private MEDIA_STATE mCurrentState = MEDIA_STATE.READY;
    ...
    public boolean init() {
        ...
        videoFilePath = Environment.getExternalStorageDirectory() + "/tianyuwenhua/";
            ...
            // ICS device
            {
                if (mSurfaceTexture == null) {
                   ...
                   *//*     AssetFileDescriptor afd = mParentActivity.getAssets()
                        ...
                        mMediaPlayer.setDataSource(videoFilePath + filename);
...
//                mPlayerHelperActivityIntent = new Intent(mParentActivity, VideoViewActivity.class);
//                mPlayerHelperActivityIntent = new Intent(mParentActivity, TextureFullscreenPlayback.class);
                mPlayerHelperActivityIntent = new Intent(mParentActivity, OpenglVideoPlayerActivity.class);
//                mPlayerHelperActivityIntent = new Intent(mParentActivity, FullscreenPlayback.class);
    ...
    {
        LogUtil.d("VideoPlayerHelper", "是否全屏播放:" + fullScreen);
        if (fullScreen) {
...
//            mPlayerHelperActivityIntent = new Intent(mParentActivity, FullscreenPlayback.class);
            ...
            mPlayerHelperActivityIntent.setFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB)
                ...
                mParentActivity.startActivityForResult(mPlayerHelperActivityIntent, REQUEST_CODE_FULL_SCREEN);
            return true;
        } else {
    ...
    public boolean stop() {
        if (!isPlayableOnTexture()) {
            // Log.d( LOGTAG,
            // "Cannot releaseMedia this video since it is not on texture");
            return false;
        }

        if ((mCurrentState == MEDIA_STATE.NOT_READY)
                || (mCurrentState == MEDIA_STATE.ERROR)) {
            // Log.d( LOGTAG, "Cannot releaseMedia this video if it is not ready");
            return false;
        }
        ...
        if (mMediaPlayer != null) {
            ...
            try {
                ...
                mMediaPlayerLock.unlock();
                Log.e(LOGTAG, "Could not releaseMedia playback");
            }
