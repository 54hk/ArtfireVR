...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    setState(State.RESET);
                    if (null != isRefreshLinstener) {
                        isRefreshLinstener.isRefresh(true);
                    }
                    return true;
                }
                break;
    ...
    // 侯坤写的下拉隐藏头布局监听
    public interface IsRefreshLinstener {
        void isRefresh(boolean isRefresh);
    }

    private IsRefreshLinstener isRefreshLinstener;

    public void setIsRefreshLinstener(IsRefreshLinstener isRefreshLinstener) {
        this.isRefreshLinstener = isRefreshLinstener;
    }
